...
export default class ReviewPage {
  ...
  async load() {
    try {
      ...
      // Cargar RAW para contexto
      await this.loadRawContext(d.raw_post_id);

      // Asegurar cover si no hay
  ...
  setCover(url: string) {
    ...
    // Si estaba excluida, re-habilitar
    const ex = new Set(this.excludedImages());
    if (ex.has(url)) {
      ex.delete(url);
  ...
  removeImage(url: string) {
    ...
    // Si quitaste la portada, mover a otra visible
    const d = this.draft()!;
